async function main() {
    ...
    async function createCard2(music) {
        for (let index = 0; index < music.length - 1; index++) {
            ...
            let Artistname = element.split("/music/")[1].replaceAll("%20", " ").split("-")[0]
            let Songname = element.split("/music/")[1].replaceAll("%20", " ").split("-")[1]
        ...
        playcard.forEach(element => {
            element.addEventListener("click", async () => {
                ...
                for (let index = 0; index < music.length; index++) {
                    ...
                    let cSongname = nowmusic.split("/music/")[1].replaceAll("%20", " ").split("-")[1]
    ...
    async function getMusic() {
        let response = await fetch("http://127.0.0.1:5501/music/")
    ...
    async function update_name(audio) {
        let lsong = audio.src.split("/music/")[1].replaceAll("%20", " ")
